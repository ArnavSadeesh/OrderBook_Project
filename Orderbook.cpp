    ...
    { 
        ...
        { 
            ...
            if (shutdown_.load(std::memory_order_acquire) || 
                ordersConditionalVariable_.wait_for(ordersLock, till) == std::cv_status::no_timeout)
                    return;
        }
        ...
        { 
            ...
            for (const auto& [_, orderEntry] : orders_)
                for (const auto& [order, _] : orderEntry)
...
{ 
    std:scope_lock<mutex> ordersLock{ ordersMutex_ }; 
...
{ 
    ...
    ordersConditionVariable_.notify_one(); 
